...
class Controller extends BaseController
{
    ...
    public function ScreenforName($input_value, $parcent){
        ...
        $screen_name = DB::table('screen_ofacs')->select('sdn_name')->where('sdn_name', 'LIKE', '%'.$input_value.'%')->get();
    ...
    function compeerString( $string1, $string2 )
    {
        ...
        public function ScreenforDob($column, $name, $year, $parcent){
            ...
            $screen_dob_result = DB::table('screen_hm_fst')->select('dob')->where($column, $name)->get();
            if(count($screen_dob_result) > 0){
                foreach($screen_dob_result as $single_value){

                    $date_of_birth = date('Y', strtotime($single_value->dob));
        ...
        public function ScreenforCountry($column, $name, $country, $parcent){
            ...
            $screen_country_result = DB::table('screen_hm_fst')->select('country')->where($column, $name)->get();
